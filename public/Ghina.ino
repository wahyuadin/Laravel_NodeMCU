...
#ifndef STASSID
#define STASSID "Ghina"
#define STAPSK "ghinafdl"
#endif
...
FuzzySet* tempDingin = new FuzzySet(0, 0, 15, 20);
FuzzySet* tempNormal = new FuzzySet(15, 20, 30, 35);
FuzzySet* tempPanas = new FuzzySet(30, 40, 100, 100);

// Deklarasi himpunan fuzzy untuk kelembaban tanah
FuzzySet* soilKering = new FuzzySet(0, 0, 30, 40);
FuzzySet* soilLembab = new FuzzySet(30, 40, 50, 60);
FuzzySet* soilBasah = new FuzzySet(50, 60, 100, 100);

// Deklarasi himpunan fuzzy untuk keputusan penyiraman
FuzzySet* tidakSiram = new FuzzySet(0, 0, 0, 50);
FuzzySet* siram = new FuzzySet(50, 100, 100, 100);
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (https.begin(*client, String("https://") + host + "/api/api.php/")) {  // HTTPS
      ...
      if (waterDecisionValue > 50) {
        digitalWrite(relay, LOW); // Hidupkan relay
